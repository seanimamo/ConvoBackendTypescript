...
describe("Test DistrictRepository", () => {
  ...
  test("Saving new talkingPoint succeeds when not checking for parent", async () => {
    await talkingPointRepo.save(talkingPoint, false);
  });
  ...
  test("Saving new talkingPoint succeeds when checking for parent and the parent exists", async () => {
    ...
    await talkingPointRepo.save(talkingPoint, true);
  });

  test("Saving new talkingPoint fails when checking for parent and the parent doesnt exist", async () => {
    await expect(talkingPointRepo.save(talkingPoint, true)).rejects.toThrow(ParentObjectDoesNotExistError);
  });

  test("Saving new talkingPoint fails when another talking point with the same id already exists", async () => {
    await districtRepo.save(district);
    await talkingPointRepo.save(talkingPoint, true);
    await expect(talkingPointRepo.save(talkingPoint, true)).rejects.toThrow(UniqueObjectAlreadyExistsError);
  });
  ...
  test("Retrieve multiple taking points by district title succeeds and only gets posts under the given district title",
    async () => {
      ...
      await talkingPointRepo.save(post1, true);
      await talkingPointRepo.save(post2, true);
      await talkingPointRepo.save(post3, true);
      await talkingPointRepo.save(post4, true);
